 ...
 * @const
 */
const IS_PROGRESS_MODE = (process.argv || []).some(
  (arg) => arg === "--progress"
);
 ...
 * @enum
 */
const LOADER_REPLACEMENT_CONDITIONS: (boolean | string)[] = [];
...
ERROR_MESSAGES[0] =
  `File ($1) doesn't exist but specified in ${LOADER_NAME} options with \n` +
...
ERROR_MESSAGES[1] =
  `File ($1) doesn't exist but specified in replacement. ${LOADER_NAME} can't replace\n` +
  `  it by '${LOADER_REPLACEMENT_CONDITIONS[5]}' condition. Make sure that replacement file exists. \n` +
  `  ${HELP_INFO_MESSAGE}`;

ERROR_MESSAGES[2] =
  `should be equal to one of the allowed values: [$1]. \n` +
  `  ${HELP_INFO_MESSAGE}`;

ERROR_MESSAGES[3] =
  `${LOADER_NAME} must executes before other loaders. Check your Webpack config file.\n` +
  `  NOTE: Webpack reads loaders from right to left. So ${LOADER_NAME} have to be the last in array of loaders. \n` +
  `  ${HELP_INFO_MESSAGE}`;

ERROR_MESSAGES[4] =
  `should be full path to file or function returning full path to file. \n` +
  `  ${HELP_INFO_MESSAGE}`;
 ...
 * @const
 */
const LOADER_OPTIONS_SCHEMA: Schema = {
  title: LOADER_NAME,
  ...
  properties: {
    condition: {
      ...
      errorMessages: {
        enum: ERROR_MESSAGES[2].replace(
          "$1",
          LOADER_REPLACEMENT_CONDITIONS.join(", ")
        ),
      },
    },
    replacement: {
